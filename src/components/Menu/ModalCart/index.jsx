...
const card = tv({
  slots: {
    container:
      "flex items-center justify-center w-full min-h-screen bg-fade-modal fixed z-99",
    containerModal:
      "w-full h-600 px-2 py-5 lg:w-800 mx-6 md:mx-8 lg:px-5 bg-box-background rounded-sm",
    containerModalInternal: "flex flex-1 flex-col items-center",
    containerContent:
      "flex flex-col justify-start items-center w-full py-2 border-t-2 border-b-2 border-border-red lg:h-450",
    modalTitle: "mb-4 text-3xl lg:text-4xl font-imbue text-primary-red",
    contentTitle:
      "lg:mb-2 text-2xl lg:text-3xl font-inter font-bold text-primary-red",
    ...
    containerPriceFinalize:
      "flex flex-row justify-between items-center w-full h-full px-1 py-4",
    containerInternalPriceFinalize: "flex flex-row items-center",
    buttonFinalize:
      "p-2 bg-primary-red hover:bg-highlight-red text-sm rounded-sm md:text-lg font-inter font-bold text-white hover:shadow-red cursor-pointer duration-200",
...
const {
  ...
  spanBox,
  containerPriceFinalize,
  containerInternalPriceFinalize,
  buttonFinalize,
  icon,
} = card();
...
*Endereço de entrega*: ${address}`;
      ...
      alert(
        "Pedido finalizado com sucesso! Aguarde o contato de um de nossos atendentes para confirmar o pagamento."
      );
      setShowModal(false);
    }
  };
  ...
  return (
    <div className={container()} onClick={() => setShowModal(false)}>
      <div className={containerModal()} onClick={(e) => e.stopPropagation()}>
        <div
          className={containerModalInternal({
            containerModalInternal: "title",
          })}
        >
          <h1 className={modalTitle()}>SEU CARRINHO</h1>

          <div
            className={containerModalInternal({
              containerModalInternal: "content",
            })}
          >
            <div className={containerContent({ containerContent: "pizza" })}>
              <h1 className={contentTitle()}>PIZZAS</h1>

              <div className={flexBox()}>
                <p className={boxTitle()}>Tamanho selecionado:</p>
              ...
              <div className={flexBox()}>
                <p className={boxTitle()}>Sabores selecionados:</p>
            ...
            <div className={containerContent({ containerContent: "drink" })}>
              <h1 className={contentTitle()}>BEBIDAS</h1>
